...
async def main():
    ...
    water_mark_file = "watermark.png"
    is_watermarkable = True
    min_duration = 60  # Minimum duration in seconds to take screenshots
    no_of_photos = 5   # Number of screenshots to generate
    wf = water_mark_file if is_watermarkable else None
    
    images = await generate_screen_shots(video_file, output_directory, is_watermarkable, wf, min_duration, no_of_photos)
    if images:
